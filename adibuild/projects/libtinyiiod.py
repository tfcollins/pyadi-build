"""libtinyiiod CMake library builder."""
...
DEFAULT_REPO_URL = "https://github.com/analogdevicesinc/libtinyiiod.git"

#: Subdirectory name used when caching the repo under ``~/.adibuild/repos/``.
REPO_CACHE_NAME = "libtinyiiod"


class LibTinyIIODBuilder(BuilderBase):
    """
    Builder for the libtinyiiod C library.

    Clones `<https://github.com/analogdevicesinc/libtinyiiod>`_ (or a
    ...
    **Output artifacts** (collected into ``get_output_dir()``)::
        ...
        libtinyiiod.so.<version>   # versioned shared library
        libtinyiiod.so.<soversion> # soname symlink
        libtinyiiod.so             # development symlink
        tinyiiod.h                 # public header
        libtinyiiod.pc             # pkg-config file
    ...
    def prepare_source(self) -> Path:
        """
        Clone or update the libtinyiiod repository.

        The repo is cached at ``~/.adibuild/repos/libtinyiiod/``.  A local path
        ...
        """
        self.logger.info("Preparing libtinyiiod source...")
    ...
    def build(
        ...
        """
        Configure and build libtinyiiod.
        ...
        self.logger.info(f"Building libtinyiiod with {jobs} job(s)...")
    ...
    def package_artifacts(self) -> list[Path]:
        ...
        Collects:
        - ``libtinyiiod.so*`` (shared library + symlinks)
        - ``tinyiiod.h`` (public header, from source root)
        - ``*.pc`` (pkg-config file)
        ...
        if not self.script_mode:
            ...
            for so_file in sorted(build_dir.glob("libtinyiiod.so*")):
            ...
            header = self.source_dir / "tinyiiod.h"
            if header.exists():
                dst = output_dir / "tinyiiod.h"
