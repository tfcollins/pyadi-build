...
def _make_config(platform_name, arch, extra=None):
    ...
    platform_cfg = {
        "arch": arch,
        "cross_compile": (
            "aarch64-linux-gnu-" if arch == "arm64" else "arm-linux-gnueabihf-"
        ),
...
class TestBootBuilder:
    def test_zynq_flow(self, tmp_path, mocker):
        ...
        mocker.patch.object(
            builder, "_ensure_uboot", return_value=Path("/tmp/u-boot.elf")
        )
    ...
    def test_versal_flow(self, tmp_path, mocker):
        ...
        assert (
            "core=a72-0, exception_level=el-3, trustzone, file=/tmp/bl31.elf"
            in bif_content
        )
    ...
    def test_build_from_source_calls_sub_builders(self, tmp_path, mocker):
        ...
        assert any(
            "xsct" in str(call) and "gen_fsbl.tcl" in str(call)
            for call in mock_execute.call_args_list
        )
        assert any(
            "xsct" in str(call) and "gen_pmufw.tcl" in str(call)
            for call in mock_execute.call_args_list
        )
    ...
    def test_versal_build_from_source(self, tmp_path, mocker):
        ...
        assert any(
            "xsct" in str(call) and "gen_plm.tcl" in str(call)
            for call in mock_execute.call_args_list
        )
        assert any(
            "xsct" in str(call) and "gen_psmfw.tcl" in str(call)
            for call in mock_execute.call_args_list
        )
