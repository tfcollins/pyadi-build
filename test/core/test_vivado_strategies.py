...
def test_strategy_selection_order(mocker):
    ...
    mock_playwright.return_value.download.side_effect = VivadoDownloadError("Playwright failed")
    mock_selenium.return_value.download.side_effect = VivadoDownloadError("Selenium failed")
...
def test_download_installer_retries_on_failure(mocker):
    ...
    mock_docker.return_value.download.side_effect = [VivadoDownloadError("F1"), success_path]
    ...
    mocker.patch("adibuild.core.vivado.VivadoInstaller._prefer_browser_download", return_value=True)
